    ...
    <>
      <div className="bg-[#FCE6F499] py-6 sm:py-8 md:py-10 lg:py-12">
        <div className="container mx-auto px-4 md:px-0">
          {/* First Div - Flex Row */}
          <div className="flex flex-col lg:flex-row w-full gap-6 md:gap-0 mb-8 md:mb-12">
            {/* Left Side - Flex Column (2/3 width) */}
            <div className="flex flex-col space-y-3 w-full lg:w-4/5 px-4 md:px-0 ">
              <h2 className="text-3xl md:text-[60px] playfair-300 text-[#0C2165] tracking-wider">
                Practical Skills.
                <br />
                Recognised Certifications.
              </h2>
              <p className="text-base md:text-[20px]  monser-400 leading-relaxed">
                Your MBA is more than a degree. It's a toolkit for your future.
              </p>
            </div>

            {/* Right Side - Single Paragraph (1/3 width) */}
            <div className="w-full lg:w-1/3 flex items-center justify-start px-4 md:px-0 md:-ml-30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
            {statsData.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-lg shadow-sm w-full flex flex-col md:flex-row items-start space-y-3 md:space-y-0 md:space-x-3 lg:space-x-4"
              >
                ...
                <div className="flex-shrink-0 w-full md:w-1/3">
                  <div className="w-full h-full">
                    <Image
                      ...
                      width={64}
                      height={64}
                      className="w-full h-32 md:h-[160px] object-cover rounded-lg border-1 border-[#ADADAD]"
                    />
                  </div>
                </div>
                <div className="flex flex-col md:flex-row items-start space-y-2 md:space-y-0 md:space-x-3 lg:space-x-4 p-3 md:p-4 w-full md:flex-1">
                  {/* Title and Paragraph - Full width on mobile, 2/3 on desktop */}
                  <div className="flex-1 w-full">
                    <h5 className="text-[20px] md:text-[24px] monser-600 mb-2 sm:mb-3 leading-tight text-[#002561]">
                      {item.title}
                    </h5>
                    <p className="text-[16px] md:text-[18px] lg:text-lg monser-400 leading-relaxed">
                      {item.description}
                    </p>
                  </div>
                </div>
